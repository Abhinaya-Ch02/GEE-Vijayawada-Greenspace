...
var img1 = ee.Image(list.get(1)).clip(ROI);
Map.addLayer(img1, falseColorVis, 'FCC Image');

// // Create an image collection with specific bands
var imgcollection = ee.ImageCollection([ee.Image(list.get(1))])
   .select(['B2', 'B3', 'B4', 'B8']);
...
print('Validation error matrix for 2019', validationAccuracy);
print('Validation accuracy for 2019', validationAccuracy.accuracy());
...
var chart = ui.Chart.image.byClass({
  ...
  scale: 30,
  classLabels: ['Greenspaces', 'Water', 'Buildup', 'Others']
}).setOptions({
...
Map.add(legend);
// // Legend for Vegetation Map
var vegLegend = ui.Panel({
  layout: ui.Panel.Layout.flow('vertical'),
  style: {position: 'bottom-right', padding: '8px'}
});

vegLegend.add(ui.Label('Legend', {fontWeight: 'bold', fontSize: '16px'}));

// // Add entries for Vegetated and Non-Vegetated areas
var vegClasses = ['Greenspace Area', 'Vegetated Area'];
var vegColors = ['grey', 'green'];

for (var i = 0; i < vegClasses.length; i++) {
  var colorBox = ui.Label({
    style: {backgroundColor: vegColors[i], padding: '8px', margin: '2px'}
  });
var label = ui.Label(vegClasses[i], {margin: '2px'});
vegLegend.add(ui.Panel([colorBox, label], ui.Panel.Layout.flow('horizontal')));
}

// // Add the Vegetation Map legend to the map
Map.add(vegLegend);


// // Title for the Classified Land Cover Map
var titlePanel = ui.Panel({
  layout: ui.Panel.Layout.flow('vertical'),
  style: {position: 'top-center', padding: '8px'}
});

var titleClassified = ui.Label('Land Cover Landuse Classification Map of Vijayawada');
titleClassified.style().set({
  fontWeight: 'bold',
  color: 'black',
  fontSize: '16px',
  margin: '5px'
});
titlePanel.add(titleClassified);

// // Add vegetation mask (Class = 0 for Greenspaces)
...
print('Vegetation area (sq.km) for 2019:', veg_sqkm);
